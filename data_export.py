...
for _p in [
    PROJECT_ROOT,
    os.path.join(PROJECT_ROOT, "caudemllmcode2arbritrage"),
    os.path.join(PROJECT_ROOT, "cllm7"),
...
try:
    try:
        ...
        from chains import CHAINS, get_enabled_chains, get_low_fee_chains
    logger.info("✓ Loaded config.chains / chains")
except ImportError:
...
try:
    try:
        ...
        from dexscreener_feed import (
            get_token_prices_by_chain,
            get_multi_token_chain_prices,
        )
    _HAS_DEXSCREENER = True
    logger.info("✓ Loaded dexscreener_feed")
except ImportError:
...
try:
    ...
    _HAS_ARB = True
    logger.info("✓ Loaded cross_chain_arbitrage")
except ImportError:
    _HAS_ARB = False
...
try:
    try:
        ...
        from gas_optimizer import is_trade_profitable, estimate_gas_cost_usd
    _HAS_GAS = True
    logger.info("✓ Loaded gas_optimizer")
except ImportError:
    _HAS_GAS = False
