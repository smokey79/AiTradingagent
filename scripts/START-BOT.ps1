# ============================================================
#  AiTradingAgent — Full Bot Startup Script
#  Run from PowerShell (no admin needed)
#
#  HOW TO RUN:
#  1. Open VS Code  (press Windows key, type "VS Code", open it)
#  2. Press Ctrl + `  (backtick) to open the terminal
#  3. Paste this line and press Enter:
#     powershell -ExecutionPolicy Bypass -File "C:\Users\AlanJ\projects\AiTradingagent\scripts\START-BOT.ps1"
# ============================================================

$projectRoot = "C:\Users\AlanJ\projects\AiTradingagent"

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  AiTradingAgent v4 — Starting Up" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# ── Step 1: Check .env exists ─────────────────────────────────
$envFile = "$projectRoot\config\.env"
if (-not (Test-Path $envFile)) {
    Write-Host "ERROR: .env file not found at $envFile" -ForegroundColor Red
    Write-Host "Copy config\.env.example to config\.env and fill in your API keys." -ForegroundColor Yellow
    exit 1
}
Write-Host "[1/5] .env found" -ForegroundColor Green

# ── Step 2: Check Ollama is running (needed for Hermes) ────────
Write-Host "[2/5] Checking Ollama (Hermes local model)..." -ForegroundColor White
try {
    $ollamaCheck = Invoke-RestMethod -Uri "http://localhost:11434/api/tags" -TimeoutSec 4
    $models = $ollamaCheck.models | Select-Object -ExpandProperty name
    if ($models -contains "hermes3") {
        Write-Host "      Ollama running. hermes3 model ready." -ForegroundColor Green
    } else {
        Write-Host "      Ollama running but hermes3 NOT found. Run: ollama pull hermes3" -ForegroundColor Yellow
        Write-Host "      (Bot will continue — hermes agent will fall back to HOLD)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "      Ollama not running. Starting it..." -ForegroundColor Yellow
    $ollamaExe = "$env:LOCALAPPDATA\Programs\Ollama\ollama.exe"
    if (Test-Path $ollamaExe) {
        Start-Process $ollamaExe -ArgumentList "serve" -WindowStyle Hidden
        Write-Host "      Ollama started (background). Waiting 5s..." -ForegroundColor Yellow
        Start-Sleep -Seconds 5
    } else {
        Write-Host "      Ollama not installed. Download from https://ollama.com" -ForegroundColor Red
        Write-Host "      (Bot will continue — hermes agent will fall back to HOLD)" -ForegroundColor Yellow
    }
}

# ── Step 3: Check Node.js ─────────────────────────────────────
Write-Host "[3/5] Checking Node.js..." -ForegroundColor White
try {
    $nodeVer = & node --version 2>&1
    Write-Host "      Node.js $nodeVer found" -ForegroundColor Green
} catch {
    Write-Host "      ERROR: Node.js not found. Install from https://nodejs.org" -ForegroundColor Red
    exit 1
}

# ── Step 4: Check node_modules ───────────────────────────────
Write-Host "[4/5] Checking dependencies..." -ForegroundColor White
if (-not (Test-Path "$projectRoot\node_modules")) {
    Write-Host "      node_modules missing. Running npm install..." -ForegroundColor Yellow
    Set-Location $projectRoot
    npm install
}
Write-Host "      Dependencies OK" -ForegroundColor Green

# ── Step 5: Choose run mode ───────────────────────────────────
Write-Host ""
Write-Host "[5/5] How do you want to run the bot?" -ForegroundColor White
Write-Host "  1 = Paper trade (safe — no real money)" -ForegroundColor Green
Write-Host "  2 = Single consensus test (BTC/USDT, then exit)" -ForegroundColor Cyan
Write-Host "  3 = PM2 background service (runs 24/7)" -ForegroundColor Yellow
Write-Host ""
$choice = Read-Host "Enter 1, 2, or 3"

Set-Location $projectRoot

switch ($choice) {
    "1" {
        Write-Host ""
        Write-Host "Starting PAPER TRADE mode (Node.js orchestrator)..." -ForegroundColor Green
        Write-Host "Press Ctrl+C to stop." -ForegroundColor Yellow
        Write-Host ""
        $env:NODE_ENV = "paper"
        node src/index.js
    }
    "2" {
        Write-Host ""
        Write-Host "Running single BTC/USDT consensus test (Python engine)..." -ForegroundColor Cyan
        Write-Host ""
        python orchestrator/consensus_engine.py --symbol BTC/USDT --paper
    }
    "3" {
        Write-Host ""
        Write-Host "Starting with PM2 (background service)..." -ForegroundColor Yellow
        $pm2Check = Get-Command pm2 -ErrorAction SilentlyContinue
        if (-not $pm2Check) {
            Write-Host "PM2 not found. Installing globally..." -ForegroundColor Yellow
            npm install -g pm2
        }
        pm2 start ecosystem.config.js --env paper
        pm2 save
        Write-Host ""
        Write-Host "Bot running in background. Useful commands:" -ForegroundColor Green
        Write-Host "  pm2 logs         <- see live output" -ForegroundColor White
        Write-Host "  pm2 status       <- check if running" -ForegroundColor White
        Write-Host "  pm2 stop all     <- stop the bot" -ForegroundColor White
        Write-Host "  pm2 restart all  <- restart after changes" -ForegroundColor White
    }
    default {
        Write-Host "Invalid choice. Run the script again." -ForegroundColor Red
    }
}
